...
import {
  MapPin, BadgeCheck, UserCheck, Calendar, Barcode, ClipboardCheck, Briefcase, GraduationCap, Building2,
} from "lucide-react";
import {
  Dialog,
  DialogContent,
...
interface AssessorRecordDialogProps {
  record: PtcacsRecord | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function AssessorRecordDialog({ record, open, onOpenChange }: AssessorRecordDialogProps) {
  return (
  ...
  return (
    <div className="flex max-h-[92vh] flex-col">
        ...
        <div className="relative flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <DialogTitle className="mt-1.5 text-2xl leading-tight font-semibold">
              {field(record, "name") || "Unnamed Assessor"}
            </DialogTitle>
      ...
      <div className="scroll-fancy overflow-y-auto px-6 pb-6 pt-4">
        <Tabs defaultValue="assessor">
          <TabsList className="w-full">
            <TabsTrigger value="assessor">
              <UserCheck className="size-3.5" /> Assessor
            </TabsTrigger>
          ...
          <TabsContent value="assessor">
            <Section>
              <InfoItem label="Name" value={field(record, "name")} className="sm:col-span-2" />
              <InfoItem label="Sex" value={field(record, "sex")} />
              <InfoItem label="Date of Birth" value={formatDate(field(record, "date_of_birth"))} />
              <InfoItem
                label="Educational Attainment"
                value={
                  field(record, "educational_attainment") ? (
              ...
              <InfoItem
                ...
                value={
                  field(record, "present_designation") ? (
                    <span className="inline-flex items-center gap-1.5">
              ...
              <InfoItem
                ...
                value={
                  <span className="inline-flex items-center gap-1.5">
                    ...
                    {formatDate(field(record, "date_of_accreditation"))}
                  </span>
                }
              />
