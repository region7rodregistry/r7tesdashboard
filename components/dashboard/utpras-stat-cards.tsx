...
import { FileCheck, ShieldCheck, Clock, CircleSlash, type LucideIcon } from "lucide-react";
import { Card } from "@/components/ui/card";
import { cn } from "@/lib/utils";
import type { UtprasStats, ValidityStatus } from "@/lib/utpras";

export type StatusFilter = "all" | ValidityStatus;

interface StatDef {
  key: keyof UtprasStats;
  status: StatusFilter;
  label: string;
  icon: LucideIcon;
  color: string;
...
const STATS: StatDef[] = [
  { key: "total", status: "all", label: "Total Programs", icon: FileCheck, color: "text-sky-600 dark:text-sky-400", bg: "bg-sky-100 dark:bg-sky-500/15", ring: "ring-sky-500" },
...
interface UtprasStatCardsProps {
  stats: UtprasStats;
  activeStatus: StatusFilter;
  onSelect: (status: StatusFilter) => void;
}

export function UtprasStatCards({ stats, activeStatus, onSelect }: UtprasStatCardsProps) {
  return (
        ...
        return (
          <motion.button
            ...
            title={
              s.status === "all"
                ? "Show all programs"
                : `Filter to ${s.label.toLowerCase()} programs`
            }
